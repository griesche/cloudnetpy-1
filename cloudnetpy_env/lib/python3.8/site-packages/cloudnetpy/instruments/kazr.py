...
class Kazr(NcRadar):
    ...
    def mask_balloon_flights(self) -> None:
        file_balloon_flights = '/home/griesche/MOSAiC/balloon_flights.csv'
        with open(file_balloon_flights,newline='') as csvfile:
            has_header = csv.Sniffer().has_header(csvfile.read(1024))
            zero = csvfile.seek(0)
            csvreader = csv.reader(csvfile,delimiter=';')
            if has_header:
                head = next(csvreader)
            balloon_flights = [[_datetime_to_hours(row[0][11:16]),_datetime_to_hours(row[1][11:16])]
                               for row in csvreader]                 
#                    [_datetime_to_hours(np.datetime64(row[0][6:10]+'-'+row[0][3:5]+'-'+row[0][:2]+'T'+row[0][11:16])),
#                                _datetime_to_hours(np.datetime64(row[1][6:10]+'-'+row[1][3:5]+'-'+row[1][:2]+'T'+row[1][11:16]+'Z'))]
#                               for row in csvreader]
        for balloon_flight in balloon_flights:
            print(balloon_flight)
            print(self.data['time'][0],self.data['time'][-1])
            print(self.data['time'].data)
            if ((self.data['time'][0] < balloon_flight[1] and self.data['time'][-1] > balloon_flight[1]) or
                (self.data['time'][-1] > balloon_flight[0] and self.data['time'][-1] < balloon_flight[1])):
                tidx_start_balloon = np.argmin(abs(self.data['time'].data-balloon_flight[0]))
                tidx_end_balloon = np.argmin(abs(self.data['time'].data-balloon_flight[1]))
                for key in self.data.keys():
                    print(key)
                    if len(self.data[key].data.shape)<2:
                        continue
                    if len(self.data[key].data.shape)==2:
                        self.data[key].data[tidx_start_balloon:tidx_end_balloon,:] = np.ones((tidx_end_balloon-tidx_start_balloon,self.data[key].data.shape[1])) * np.nan

def _datetime_to_hours(time_str) -> float:
    return float(time_str[:2]) + float(time_str[3:])/60
