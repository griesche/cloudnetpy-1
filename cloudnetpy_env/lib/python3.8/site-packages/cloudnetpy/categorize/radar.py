...
class Radar(DataSource):
    ...
    def interpolate_to_grid(self, time_new: np.ndarray, height_new: np.ndarray) -> list:
        ...
        for key in self.data.keys():
            ...
            data_interpolated = interpolate_2d_nearest(
                self.time[indices], self.height, data[indices, :], time_new, height_new
            )
...
#            max_time /= 60  # to fraction hour
            ...
            if bad_time_indices:
                logging.warning(f"Unable to interpolate radar for {len(bad_time_indices)} time steps")
            data_interpolated[bad_time_indices, :] = ma.masked
            if bad_height_indices:
                logging.warning(f"Unable to interpolate lidar for {len(bad_height_indices)} altitudes")
            data_interpolated[:, bad_height_indices] = ma.masked
            self.data[key].data = data_interpolated
