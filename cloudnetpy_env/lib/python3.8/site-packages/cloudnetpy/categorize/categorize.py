...
def generate_categorize(input_files: dict, output_file: str, uuid: Optional[str] = None) -> str:
    ...
    def _interpolate_to_cloudnet_grid() -> list:
        ...
        radar_a3k_data_gap_indices = data_a3k["radar"].interpolate_to_grid(time, height)
        lidar_data_gap_indices = data["lidar"].interpolate_to_grid(time, height)
        lidar_a3k_data_gap_indices = data_a3k["lidar"].interpolate_to_grid(time, height)
        bad_time_indices = list(set(radar_data_gap_indices + lidar_data_gap_indices + radar_a3k_data_gap_indices + lidar_a3k_data_gap_indices))
    ...
    def _merge_b3k_a3k():
        ...
        for key in data["radar"].data.keys():
            ...
            if key == 'nyquist_velocity':
                continue
#                merged_array = np.concatenate((data["radar"].data[key][0],data_a3k["radar"].data[key][0]))
#            else:
#            merged_array = np.concatenate((data["radar"].data[key][:,:95],data_a3k["radar"].data[key][:,95:]),axis=1)
#            data["radar"].data[key].data.data[:,:] = merged_array
            data["radar"].data[key].data[:,95:] = data_a3k["radar"].data[key].data[:,95:]
        for key in data["lidar"].data.keys():
            print(key)
            if key in ['lidar_wavelength','beta_error','beta_bias']:
                continue
#            merged_array = np.concatenate((data["lidar"].data[key][:,:95],data_a3k["lidar"].data[key][:,95:]),axis=1)
#            data["lidar"].data[key].data.data[:,:] = merged_array
            data["lidar"].data[key].data[:,30:] = data_a3k["lidar"].data[key].data[:,30:]
